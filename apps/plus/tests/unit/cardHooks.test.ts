...
const image: ImageCardData = {
    imageId: 2, boardId: 5, publicId: "public", secureUrl: "url", fileName: "image.png",
...
describe("card hooks", () => {
    ...
    describe("useImageCard", () => {
        it("uses the latest drag state when saving a persisted image", () => {
            ...
            act(() => vi.runAllTimers());
            expect(onUpdate).toHaveBeenCalledWith(2, 5, "public", "url", "image.png", 51, 60, 4, 400, 300);
            expect(onEditingClear).toHaveBeenCalledOnce();
        });

        it("inserts temporary image files and does nothing without a file", () => {
            const onInsert = vi.fn();
            const base = {
                canEdit: true, isEditing: true, onEditing: vi.fn(), onEditingClear: vi.fn(),
                onPermissionDenied: vi.fn(), onInsert, onUpdate: vi.fn(), onDelete: vi.fn(),
            };
            const withFile = renderHook(() => useImageCard({ image: { ...image, imageId: -1, file: imageFile }, ...base }));
            act(() => dispatchBoardPress("pointerup"));
            act(() => vi.runAllTimers());
            expect(onInsert).toHaveBeenCalledWith(-1, imageFile, 5, 10, 20, 4, 400, 300);
            withFile.unmount();

            onInsert.mockClear();
            renderHook(() => useImageCard({ image: { ...image, imageId: -2 }, ...base }));
            act(() => dispatchBoardPress("pointerup"));
            act(() => vi.runAllTimers());
