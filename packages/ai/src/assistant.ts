...
/**
 * 제품마다 달라지는 프롬프트 조각. 규칙과 도구 정의는 두 제품이 같아야 하지만,
 * "이 제품이 무엇인가"와 "이미지를 어떻게 넣는가"는 다르므로 앱이 넣어 준다.
 */
export type AssistantProduct = {
    /** 첫 문단. 이 제품이 무엇이고 무엇이 없는지 알려 준다. */
    intro: string[];
    /** 그림 요청을 어떻게 돌려보낼지. 이미지를 넣는 방법이 앱마다 다르다. */
    imageRules: string[];
    /** 사용법 질문에 어떻게 답할지. 들 수 있는 예가 앱마다 다르다. */
    usageRules: string[];
    /** 제품 사용법 본문. */
    guide: string;
};
...
export const createAssistantSystemPrompt = (product: AssistantProduct) => [
    ...product.intro,
    "",
    "규칙:",
    ...
    "- 최종 문서 순서는 사용자가 정한 메모 순서다. 재배치는 좌표만 바꾸므로 문서 순서를 바꿀 수 없다.",
    "",
    "카드 고치기와 지우기:",
...
export async function runBoardAssistant(
    ...
    const request = {
        ...
        config: {
            systemInstruction: [
                createAssistantSystemPrompt(product),
                "",
