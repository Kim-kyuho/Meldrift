...
소스: `packages/ui/src/features/board/BoardSearchPanel.tsx`
...
결과 목록은 `useBoardSearch`가 메모를 `sort_order` 오름차순으로 정렬한 뒤 필터링한다. 이전/다음이 도는 차례와 검색어 입력 시 처음 포커스되는 메모 모두 이 순서를 따르므로, `BoardNavigator`의 연번과 어긋나지 않는다.
