...
| `canEdit` | `boolean` (선택, 기본 `true`) | `editMermaid` 권한 체크. 권한 개념이 없는 Free Edition은 넘기지 않는다 |
...
| `onPermissionDenied` | `() => void` (선택, 기본 no-op — `packages/ui/src/internal/noop.ts`) | `canEdit`가 false일 때 |
