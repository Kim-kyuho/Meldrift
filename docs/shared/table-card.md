...
| `canEdit` | `boolean` (선택, 기본 `true`) | `useTableCard`에 전달 → `editTable`의 권한 체크. 권한 개념이 없는 Free Edition은 넘기지 않는다 |
...
| `onPermissionDenied` | `() => void` (선택, 기본 no-op — `packages/ui/src/internal/noop.ts`) | `editTable`이 `canEdit === false`일 때 호출 |
